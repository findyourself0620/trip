...
export function HomeView() {
  ...
  return (
    <div className="h-full bg-background flex flex-col relative overflow-hidden">
      {/* Hero Section */}
      <div className="relative h-[45vh] min-h-[200px] overflow-hidden">
        {/* Back Button */}
        <Link href="/">
        ...
        <div className="absolute top-4 left-16 z-50">
          {/* Placeholder for potential future header elements if needed */}
        </div>
        <motion.div
          initial={{ scale: 1.1 }}
          animate={{ scale: 1 }}
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-transparent to-background flex flex-col justify-end p-8 pb-12 pt-20">
          <motion.div
          ...
          >
            ...
            <p className="text-white/90 text-sm flex items-center font-light tracking-wider">
              <Map size={16} className="mr-2 opacity-80" /> {itinerary.location}
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="flex-1 px-6 relative -mt-8 pb-24 z-10">
        ...
        >
          ...
          <ul className="space-y-4 mb-8">
            {/* Render summary from trip data config */}
            {itinerary.summary.map((item, idx) => (
              <motion.li
                key={idx}
          ...
          <div className="flex flex-col gap-3">
            ...
            {itinerary.config.albumUrl && (
              ...
              >
                <Image size={18} className="mr-2" /> 拾光影像集
              </button>
            )}
          </div>
        </motion.div>
